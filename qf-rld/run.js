...
console.info('NODE_PATH: ' + process.env.NODE_PATH)
...
tasks.set('html', () => {
  ...
  const template = fs.readFileSync('./app/index.ejs', 'utf8');
  const render = ejs.compile(template, { filename: './app/index.ejs' });
  const output = render({ debug: webpackConfig.debug, bundle: assets.main.js, config });
  fs.writeFileSync('../Content/foo.html', output, 'utf8');
});
...
//
// Fetch transport layers
// -----------------------------------------------------------------------------
tasks.set('fetch', () => {
  const getLayers = require('layers-fetcher');
  getLayers({
    fname: './app/api/layers.js',
    hex: 1
  })
  /*
  getLayers({
    fname: './app/api/layersFull.js',
    hex: 1,
    full: 1
  })
  */
...
tasks.set('start', () => {
  ...
  var proxy = httpProxy.createProxyServer({
    target: 'http://localhost:60221/'
  });

  var proxyMiddleware = function(req, res, next) {
    if (req.url.indexOf('/workspace/') === 0) {
      console.info(req.url)
      proxy.web(req, res, {target: 'http://localhost:60221/'});
  ...
  */
  ...
  var proxyMiddleware = proxy(['/account', '/consult', '/content'], {
    target: 'http://localhost:60221',
    changeOrigin: true
  });
